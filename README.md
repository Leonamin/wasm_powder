...
## 2. 역할 분담
-   **이승민:** 프로젝트 전반적인 아키텍처 설계, C++ 입자 시뮬레이션 코어 구현, 메모리 최적화.
-   **강은별:** 물리 엔진 개선(중력, 밀도 기반 이동 알고리즘), 화학 반응 시스템 설계.
-   **김지민:** 나무 + 불 화학 반응 구현, UI 개선. 
## 2. 프로젝트 개요 및 주요 기능

### 아키텍처 (Architecture)

![WASM Power Architecture](docs/WASM_Power_Architecture.png)
...
- **다양한 물질 구현:** 고체(모래, 벽), 액체(물, 기름), 기체(증기, CO2), 특수(불) 등 다양한 상태의 물질을 시뮬레이션합니다.
- **화학 반응 시스템:** 물질 간의 상호작용을 구현했습니다. (예: 불이 나무를 태움)
- **고성능 렌더링:** WASM 메모리에 직접 접근(Shared Memory)하여 데이터를 복사하는 오버헤드를 최소화했습니다.
- **사용자 도구:** 브러시 크기/모양 조절 제어 기능을 제공합니다.
...
### C. 물 흐름이 부자연스러운 현상
- **문제:** 물, 증기, 모래 등의 입자가 랜덤 요소 없이 항상 같은 방향으로만 이동하여 수직으로 떨어지거나, 물이 수평을 맞추지 못하고 곡선 형태로 유지되는 문제가 있었습니다.
- **원인:** 이동 로직에서 좌우 방향 선택이 고정되어 있어(항상 왼쪽 먼저 체크) 자연스러운 확산이 이루어지지 않았고, 수평 확산 거리가 1칸으로 제한되어 있었습니다.
- **해결:** 
  - 모든 입자의 대각선/수평 이동 방향을 `rand() % 2`로 랜덤 선택하도록 변경
  - 액체의 수평 확산 거리를 5칸으로 확대하여 더 빠르게 수평을 맞추도록 개선
  - 기체(증기)에 70% 확률로 상승, 30% 확률로 수평 확산하는 로직 추가
  - 얼음이 물보다 밀도가 낮아 물 위로 떠오르도록 특수 처리 추가
...
## 7. 성능 비교: WASM vs Pure JavaScript

### 화학 반응 시뮬레이션 성능 측정
**테스트 시나리오**: 기름(OIL) 40,000개 입자 + 불(FIRE) 연소 반응

| 구현 방식 | 입자 수 | 평균 FPS | 프레임 타임 (ms) | 성능 비율 |
|-----------|---------|----------|------------------|-----------|
| **WASM (C++)** | 40,000 | 60.0 | 16.7 ms | 100% (기준) |
| **Pure JavaScript** | 40,000 | 45.0 | 22.2 ms | 75% |

### 성능 차이 분석
- **WASM 우위**: 화학 반응 시스템이 포함된 복잡한 시뮬레이션에서 WASM이 약 **33% 더 빠른** 성능을 보임
- **병목 구간**: 화학 반응 체크 (8방향 이웃 순회 + 확률 계산 + 객체 생성)
- **JS의 한계**: 
  - 이중 루프 (입자 × 8방향)에서 객체 생성/복사 오버헤드
  - 동적 타입 체크 및 가비지 컬렉션 부담
- **WASM의 강점**: 
  - 메모리 직접 접근 및 정적 타입
  - 컴파일 타임 최적화
  - Zero-copy 렌더링
